...
const channel_color = "rwth/SHMviewer/#";
const channel_info = "gao/testtopic/#";

// Initialize new Paho client connection
let client = new Paho.MQTT.Client(host, Number(port), clientID);

let message_from_mqtt = "fortest";
...
function onMessageArrived(message) {
    console.log(message);
    let reg_info = new RegExp(channel_info.substring(0, channel_info.length - 1)+"*");
    let reg_color = new RegExp(channel_color.substring(0, channel_color.length - 1)+"*");
    let msg = JSON.parse(message.payloadString);

    //JSON object with two values extracted to elementID/selObj and color
    if (message.destinationName.match(reg_info)!==null) {
        console.log("message for info");
        update_info(msg);
    } else if(message.destinationName.match(reg_color)!==null){
        console.log("message for color");
        try {
            console.log(message.payloadString);
            message_from_mqtt = JSON.parse(message.payloadString);
            //console.log(msg);
