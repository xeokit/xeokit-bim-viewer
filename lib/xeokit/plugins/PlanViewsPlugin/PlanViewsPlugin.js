...
const EMPTY_THUMBNAIL = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e6kgAAAABJRU5ErkJggg==";
...
class PlanViewsPlugin extends Plugin {
    ...
    /**
     * Gets a PNG thumbnail of the given plan view.
     * @param planViewId
     * @returns {*}
     */
    getThumbnail(planViewId) {
        ...
        // TODO: Invalidate thumbnails when #size updated
        return this._thumbnails[planViewId] || (this._thumbnails[planViewId] = this._createThumbnail[planViewId]);
    }

    _createThumbnail(planViewId) {
        const viewer = this.viewer;
        const scene = viewer.scene;
        this._objectsMemento.saveObjects(scene);
        this._cameraMemento.saveCamera(scene);
        this.showObjects(planViewId);
        this._gotoThumbnailCamera();
        scene.render(true); // Force-render a frame
        const thumbnail = viewer.getSnapshot({
            width: this._size[0],
            height: this._size[1],
            format: "png",
        });
    ...
    _gotoThumbnailCamera() {
        ...
        viewer.cameraFlight.jumpTo({eye: eye2, look: look2, up: up2});
        viewer.camera.customProjection.matrix = math.orthoMat4c(aabb[0], aabb[3], aabb[1], aabb[4], aabb[2], aabb[5], tempMat4);
    ...
    showObjects(planViewId) {
        ...
        const scene = viewer.scene;
        const metaScene = viewer.metaScene;
        const storeyMetaObject = metaScene.metaObjects[planViewId];
